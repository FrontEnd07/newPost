...
const generatePaginationItems = ({ total, perPage, currentPage, lastPage, handlerTable }) => {
    ...
    items.push(
        <li key="first" className={`page-item ${currentPage === 1 ? 'disabled' : ''}`}>
            <a onClick={() => handlerTable({ page: 1 })} className="page-link" role="button" tabIndex="0">
    ...
    items.push(
        <li key="previous" className={`page-item ${currentPage === 1 ? 'disabled' : ''}`}>
            <a onClick={() => handlerTable({ page: currentPage - 1 })} className="page-link" role="button" tabIndex="0">
                <span aria-hidden="true">‹</span>
    ...
    for (let i = 1; i <= lastPage; i++) {
        items.push(
            <li key={i} className={`page-item ${i === currentPage ? 'active' : ''}`}>
                <a onClick={() => handlerTable({ page: i })} className="page-link" role="button" tabIndex="0">
                    {i}
                </a>
            </li>
        );
    ...
    items.push(
        <li key="last" className={`page-item ${currentPage === lastPage ? 'disabled' : ''}`}>
            <a onClick={() => handlerTable({ page: lastPage })} className="page-link" role="button" tabIndex="0">
                <span aria-hidden="true">»</span>
